// Your task:
// 1. Create new array with 3 elements: 'abc', 'def' and 'gh'
// 2. Add new element to array: 'xyz'
// 3. Print on console whole array
// 4. Concat (join) your array with new one: [1, 2, 3]
// 5. Print on console new array
// 6. Replace third ([2]) item in array with new value '555'
// 7. Print on console new array

// to test your solution in terminal You can run following command:
// npm run es1e6

//// TODO:
// here place your solution:

const stringArrayOne = ['abc', 'def', 'gh']
console.log(stringArrayOne)
stringArrayOne.push('xyz')
console.log(stringArrayOne)

const numberArrayOne = [1, 2, 3]
const newArray = stringArrayOne.concat(numberArrayOne)
const newArraySpreadOperator = [...stringArrayOne, ...numberArrayOne, ...newArray, ...numberArrayOne]
const copyArray = [...newArray];
console.log(newArray)
console.log('concat spread', newArraySpreadOperator)
console.log('copy by spread', copyArray)

newArray[2] = '555'
console.log(newArray)







//// DONT MODIFY CODE BELOW!
// Here You will find expected result of exercise

// Expected output:
// On console You should get:
// ['abc', 'def', 'gh', 'xyz']
// ['abc', 'def', 'gh', 'xyz', 1, 2, 3]
// ['abc', 'def', '555', 'xyz', 1, 2, 3]